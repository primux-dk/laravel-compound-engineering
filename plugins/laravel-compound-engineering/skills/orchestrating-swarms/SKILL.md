...
From the `laravel-compound-engineering` plugin (examples):

### Review Agents
```javascript
// Security review
Task({
  subagent_type: "laravel-compound-engineering:review:security-sentinel",
...
// Performance review
Task({
  subagent_type: "laravel-compound-engineering:review:performance-oracle",
...
// Laravel code review
Task({
  subagent_type: "laravel-compound-engineering:review:taylor-otwell-reviewer",
  description: "Laravel review",
  prompt: "Review this Laravel code for best practices"
})

// Architecture review
Task({
  subagent_type: "laravel-compound-engineering:review:architecture-strategist",
...
// Code simplicity
Task({
  subagent_type: "laravel-compound-engineering:review:code-simplicity-reviewer",
  description: "Simplicity check",
  prompt: "Check if this implementation can be simplified"
})
```

**All review agents from laravel-compound-engineering:**
...
- `taylor-otwell-reviewer` - Taylor Otwell's Laravel style
...
Task({
  subagent_type: "laravel-compound-engineering:research:best-practices-researcher",
...
Task({
  subagent_type: "laravel-compound-engineering:research:framework-docs-researcher",
  description: "Research Active Storage",
  prompt: "Gather comprehensive documentation about Laravel file uploads"
})

// Git history analysis
Task({
  subagent_type: "laravel-compound-engineering:research:git-history-analyzer",
...
```javascript
Task({
  subagent_type: "laravel-compound-engineering:design:figma-design-sync",
...
```javascript
Task({
  subagent_type: "laravel-compound-engineering:workflow:bug-reproduction-validator",
...
for (const file of ["User.php", "Payment.php", "ApiController.php", "OrderService.php"]) {
  TaskCreate({
    subject: `Review ${file}`,
...
Task({
  ...
  prompt: `
    ...
    Focus on the UserController endpoints.
  `
})
```
...
TaskCreate({
  ...
  description: "Extract authentication methods to AuthenticatesUsers trait",
  activeForm: "Refactoring User model..."
})

TaskCreate({
  subject: "Refactor LoginController",
  description: "Update to use new AuthenticatesUsers trait",
  activeForm: "Refactoring LoginController..."
})
...
Task({
  ...
  subagent_type: "general-purpose",
  prompt: "Claim task #2, refactor the LoginController, complete when done",
  run_in_background: true
})
...
prompt: `
  ...
  2. Check all Eloquent relationships have proper eager loading
